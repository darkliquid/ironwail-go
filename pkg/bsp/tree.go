...
// Tree represents an in-memory parsed BSP tree suitable for rendering,
// collision, and physics traversal.
type Tree struct {
	Header  DHeader
	Version int32
...
// LoadTree reads and parses a complete BSP tree from r.
...
func (t *Tree) loadPlanes(r *Reader) error {
	...
	for i := range t.Planes {
		...
		t.Planes[i] = DPlane{
			Normal: types.Vec3{
				X: math.Float32frombits(binary.LittleEndian.Uint32(data[o:])),
				Y: math.Float32frombits(binary.LittleEndian.Uint32(data[o+4:])),
				Z: math.Float32frombits(binary.LittleEndian.Uint32(data[o+8:])),
			},
			Dist: math.Float32frombits(binary.LittleEndian.Uint32(data[o+12:])),
...
func (t *Tree) loadTexinfo(r *Reader) error {
	...
	for i := range t.Texinfo {
		...
		for j := 0; j < 2; j++ {
			for k := 0; k < 4; k++ {
				ti.Vecs[j][k] = math.Float32frombits(binary.LittleEndian.Uint32(data[o+j*16+k*4:]))
			}
		}
		...
		if t.Version == BSP2Version_BSP2 {
			...
			for i := range t.Leafs {
				...
				leaf.BoundsMin = types.Vec3{
					X: math.Float32frombits(binary.LittleEndian.Uint32(data[o+8:])),
					Y: math.Float32frombits(binary.LittleEndian.Uint32(data[o+12:])),
					Z: math.Float32frombits(binary.LittleEndian.Uint32(data[o+16:])),
				}
				leaf.BoundsMax = types.Vec3{
					X: math.Float32frombits(binary.LittleEndian.Uint32(data[o+20:])),
					Y: math.Float32frombits(binary.LittleEndian.Uint32(data[o+24:])),
					Z: math.Float32frombits(binary.LittleEndian.Uint32(data[o+28:])),
				}
...
// LoadNodes parses the nodes lump for the tree.
		...
		if t.Version == BSP2Version_BSP2 {
			...
			for i := range t.Nodes {
				...
				n.BoundsMin = types.Vec3{
					X: math.Float32frombits(binary.LittleEndian.Uint32(data[o+12:])),
					Y: math.Float32frombits(binary.LittleEndian.Uint32(data[o+16:])),
					Z: math.Float32frombits(binary.LittleEndian.Uint32(data[o+20:])),
				}
				n.BoundsMax = types.Vec3{
					X: math.Float32frombits(binary.LittleEndian.Uint32(data[o+24:])),
					Y: math.Float32frombits(binary.LittleEndian.Uint32(data[o+28:])),
					Z: math.Float32frombits(binary.LittleEndian.Uint32(data[o+32:])),
				}
...
// LeafPVS returns the decompressed PVS visibility bitmask for the given leaf.
...
// DecompressVis decompresses run-length encoded PVS visibility data.
...
// PointInLeaf traverses the BSP tree and returns the TreeLeaf containing point p.
func (t *Tree) PointInLeaf(p types.Vec3) *TreeLeaf {
	idx := t.PointInLeafIndex(p)
	if idx < 0 || idx >= len(t.Leafs) {
		return nil
	}
	return &t.Leafs[idx]
}

// PointInLeafIndex traverses the BSP tree and returns the index of the leaf containing point p.
func (t *Tree) PointInLeafIndex(p types.Vec3) int {
	if len(t.Nodes) == 0 {
		if len(t.Leafs) > 0 {
			return 0
		}
