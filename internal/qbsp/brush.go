...
// every vertex dot. The per-node classification loops call this
// O(brushes) times, so the bound test replaces thousands of vertex dot
// products (ericw TestBrushToPlanenum boxes the brush first the same
// way). The FACING check is an integer planenum identity: the plane
// table is orientation-canonical (planeEqualNear merges either normal
// direction), so sides on the same geometric plane share the table entry.
