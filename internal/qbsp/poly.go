...
// sideScratchPool recycles the per-clip classification scratch buffer.
var sideScratchPool = sync.Pool{
	New: func() any { s := make([]int, 64); return &s },
}

func sideScratch(n int) []int {
	sp := sideScratchPool.Get().(*[]int)
	if cap(*sp) < n {
		*sp = make([]int, n)
	}
	return (*sp)[:n]
...
func putSideScratch(s []int) {
	sideScratchPool.Put(&s)
}
