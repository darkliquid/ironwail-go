...
type RenderData struct {
	...
	Lightmaps     []worldimpl.WorldLightmapPage
	HasLitWater   bool
	BoundsMin     [3]float32
	BoundsMax     [3]float32
	TotalVertices int
...
func BuildWorldGeometry(tree *bsp.Tree) (*worldimpl.WorldGeometry, error) {
	return BuildModelGeometry(tree, 0)
}

// BuildModelGeometry extracts renderable geometry for a specific BSP model index.
func BuildModelGeometry(tree *bsp.Tree, modelIndex int) (*worldimpl.WorldGeometry, error) {
	if tree == nil {
	...
	geom := &worldimpl.WorldGeometry{
		Vertices: make([]worldimpl.WorldVertex, 0, 4096),
		Indices:  make([]uint32, 0, 16384),
		Faces:    make([]worldimpl.WorldFace, 0, 256),
		Tree:     tree,
	}
	lightmapAllocator, err := surfaceimpl.NewLightmapAllocator(LightmapPageSize, LightmapPageSize, false)
	if err != nil {
		return nil, fmt.Errorf("create lightmap allocator: %w", err)
	}
	lightmapPages := make([]worldimpl.WorldLightmapPage, 0, 4)

	textureMeta := worldimpl.ParseTextureMeta(tree)
	...
	for faceIdx := 0; faceIdx < numFaces; faceIdx++ {
		...
		if int(face.Texinfo) >= 0 && int(face.Texinfo) < len(tree.Texinfo) {
			...
			textureFlags = worldimpl.DeriveFaceFlags(textureType, tree.Texinfo[face.Texinfo].Flags)
		}
		faceData := worldimpl.WorldFace{
...
func ExtractFaceVertices(tree *bsp.Tree, face *bsp.TreeFace, textureMeta []worldimpl.TextureMeta, allocator *surfaceimpl.LightmapAllocator, pages *[]worldimpl.WorldLightmapPage) ([]worldimpl.WorldVertex, *FaceLightmapSurface, error) {
...
func BuildWorldRenderData(tree *bsp.Tree) (*RenderData, error) {
	return BuildModelRenderData(tree, 0)
}

// BuildModelRenderData builds render data for a specific BSP submodel index.
func BuildModelRenderData(tree *bsp.Tree, modelIndex int) (*RenderData, error) {
	...
	renderData := &RenderData{
		Geometry:      geom,
		Lightmaps:     append([]worldimpl.WorldLightmapPage(nil), geom.Lightmaps...),
...
func faceCenter(vertices []worldimpl.WorldVertex) [3]float32 {
	if len(vertices) == 0 {
		return [3]float32{}
	}
...
func faceHasLitWater(textureFlags int32, lightmapSurface *FaceLightmapSurface) bool {
...
func assignFaceLightmap(vertices []worldimpl.WorldVertex, rawCoords [][2]float32, face *bsp.TreeFace, tree *bsp.Tree, allocator *surfaceimpl.LightmapAllocator, pages *[]worldimpl.WorldLightmapPage) (*FaceLightmapSurface, error) {
	...
	for len(*pages) <= texNum {
		*pages = append(*pages, worldimpl.WorldLightmapPage{Width: LightmapPageSize, Height: LightmapPageSize})
	}
	...
	samples := worldimpl.ExpandLightmapSamples(tree.Lighting, tree.LightingRGB, int(face.LightOfs), sampleSize8)
	if samples == nil {
		return nil, nil
	}

	(*pages)[texNum].Surfaces = append((*pages)[texNum].Surfaces, worldimpl.WorldLightmapSurface{
		X:       x,
		Y:       y,
		Width:   smax,
		Height:  tmax,
