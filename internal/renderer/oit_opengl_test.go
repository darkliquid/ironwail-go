...
func TestDestroyOITFramebuffersZeroValueNoop(t *testing.T) {
	...
	if r.oitFB.samples != 0 {
		t.Fatalf("destroyOITFramebuffers left non-zero samples: %d", r.oitFB.samples)
	}
}
...
func TestDestroyOITFramebuffersZeroesDimensions(t *testing.T) {
	...
	if r.oitFB.samples != 0 {
		t.Fatalf("destroyOITFramebuffers did not reset samples: %d", r.oitFB.samples)
	}
}
