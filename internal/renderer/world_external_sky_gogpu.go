...
func (r *Renderer) createWorldExternalSkyFaceTexture(device *wgpu.Device, queue *wgpu.Queue, label string, rgba []byte, width, height int) (*wgpu.Texture, *wgpu.TextureView, error) {
	...
	writeStart := time.Now()
	slog.Info("external sky texture write begin", "subsystem", externalSkyboxLogSubsystem, "name", r.worldSkyExternalName, "label", label, "width", width, "height", height, "rgba_bytes", len(rgba), "bytes_per_row", width*4, "rows_per_image", height)
	if err := queue.WriteTexture(&wgpu.ImageCopyTexture{
		...
		texture.Release()
		slog.Warn("external sky texture write failed", "subsystem", externalSkyboxLogSubsystem, "name", r.worldSkyExternalName, "label", label, "width", width, "height", height, "rgba_bytes", len(rgba), "error", err, "elapsed_ms", elapsedMilliseconds(writeStart))
