...
// TraceGround casts a straight-down ray from origin up to maxDist units and
// returns the Z coordinate where it first crosses from an empty/liquid leaf
// into a solid leaf. Returns (0, false) if the ray never hits solid.
//
// The trace mirrors the C R_RecursiveLightPoint walk used for alias shadow
// placement, but ignores lightmap sampling since callers only need a ground
// plane Z.
func (t *Tree) TraceGround(origin [3]float32, maxDist float32) (float32, bool) {
	if t == nil || len(t.Nodes) == 0 || maxDist <= 0 {
		return 0, false
	}
	end := origin
	end[2] -= maxDist
	return t.traceGroundNode(TreeChild{Index: 0}, origin, end)
}

func (t *Tree) traceGroundNode(child TreeChild, start, end [3]float32) (float32, bool) {
	if child.IsLeaf {
		return 0, false
	}
	if child.Index < 0 || child.Index >= len(t.Nodes) {
	...
	if node.PlaneNum < 0 || int(node.PlaneNum) >= len(t.Planes) {
		return 0, false
	}
	plane := &t.Planes[node.PlaneNum]
	...
	if plane.Type < 3 {
		...
		front = start[0]*plane.Normal[0] + start[1]*plane.Normal[1] + start[2]*plane.Normal[2] - plane.Dist
		back = end[0]*plane.Normal[0] + end[1]*plane.Normal[1] + end[2]*plane.Normal[2] - plane.Dist
	}

	if front >= 0 && back >= 0 {
		return t.traceGroundNode(node.Children[0], start, end)
	}
	if front < 0 && back < 0 {
		return t.traceGroundNode(node.Children[1], start, end)
	}
	...
	var nearChild, farChild TreeChild
	if front >= 0 {
	...
	if z, ok := t.traceGroundNode(nearChild, start, mid); ok {
		return z, ok
	}

	if farChild.IsLeaf && farChild.Index >= 0 && farChild.Index < len(t.Leafs) {
		if t.Leafs[farChild.Index].Contents == ContentsSolid {
			return mid[2], true
		}
	}

	return t.traceGroundNode(farChild, mid, end)
}
