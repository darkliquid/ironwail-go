...
- **Hickman, Zachary.** *"Quake Engine Analysis."* Northeastern University.
  - Local PDF: `article/analysisfinal.pdf`
  - Local text extraction: `article/analysisfinal.txt`
...
- **README:** `README.md` — Stated intent, "AI slop" framing, feature flags, parity goals.
- **AGENTS:** `AGENTS.md` — Agentic engineering guidelines, Senior-Junior partnership model, gotchas, build/test commands.
- **Doc Go (Root):** `doc.go` — Root architecture diagram and subsystem overview.
- **Learning Guide:** `docs/LEARNING_GUIDE.md` — Package map, client/server mental model.
- **Comparison Doc:** `docs/COMPARISON.md` — Structural C-vs-Go divergence summary.
- **Parity Guide:** `docs/PARITY.md` — Canonical parity status, `qbj3_stickflip` evidence workflow, sign-off rules.
- **Renderer Learning Plan:** `docs/RENDERER_LEARNING_PLAN.md` — 14-stage renderer curriculum.
- **Vertex Layout Contract:** `docs/VERTEX_LAYOUT.md` — The 48-byte `WorldVertex` specification.
- **Quake Specification:** `docs/QUAKE_SPECIFICATION.md` — Formal engine behavior specification (FS, cvars, physics, net).
- **QCVM Entity Sync Guide:** `docs/QCVM_ENTITY_SYNC.md` — Dual-storage VM sync problem, bug chain, unified sync architecture.
- **QuakeGo Guide:** `docs/QGO_QUAKEGO_GUIDE.md` — QuakeGo subset, compiler workflow, side-project boundaries.
- **Per-Package Docs:** `docs/internal/*.md` (`qc.md`, `renderer.md`, `server.md`, `host.md`, `client.md`, `fs.md`, `bsp.md`, `net.md`, `cmdsys.md`, `audio.md`, `engine.md`, `async.md`).
- **Diagnoses:**
  - `docs/diagnoses/qbj2_water.md` — Water translucency, Vulkan discard, uniform offset diagnosis.
  - `docs/diagnoses/qbj2_materials.md` — Texture atlas materials buffer capacity diagnosis.

---

## Issue Transcripts and Upstream Bug Reports

Captured in `article/gogpu_issues.md` (fetched from `gogpu/gogpu` repository on 2026-07-27):
