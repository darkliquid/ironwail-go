...
```bash
mise run ai-dlc "your idea or prompt"
```

(`mise.toml` task definition is provided below in "Mise Task Wiring".)
...
6. **Scale & performance** — Volumes, latencies, multi-tenancy ceiling.
7. **Security** — AuthN/Z, tenant isolation (RLS), encryption, secret handling, audit trail.
...
3. **Information architecture / topology** (UI tree, NATS subjects, RPC surface, DB schema).
4. **Database Schema & Data Models** — PostgreSQL 16 + RLS, `org_id` everywhere.
5. **State machines / flows** (Mermaid where the repo uses it).
6. **Security model** — authz, RLS, tenant isolation, audit.
...
- RLS / tenant isolation claimed but not designed (schema gaps).
- Acceptance criteria not obviously verifiable.
- Interface contracts (proto fields, NATS subjects) that would break existing consumers.
...
- What constrains us? (existing ADRs, stack in AGENTS.md §3, licensing, security §3.4)
...
- **Per task:** Red-Green TDD steps: write failing test (RED) -> minimal code (GREEN) -> repeatableactor -> verify; exact file paths; test commands (repo: `go test -v -race ./pkg/...`); acceptance criteria from the spec; dependencies on other tasks/phases.
...
- **Epic collapse**: if the run is itself an epic, create the epic bead first and attach dependencies to it, mirroring the repo style (e.g. `[epic] Org Admin Console & Self-Service [SPEC-017]`).
...
Relevant existing specs/ADRs/architecture non-negotiables (AGENTS.md section 3), prior attempts, and any partial answers.
...
For a `mise run ai-dlc "..."` entrypoint, add to `mise.toml`:

```toml
[tasks.ai-dlc]
description = "Run the AI-DLC lifecycle on a prompt (spec -> review -> ADRs -> review -> plan -> review -> beads)"
run = ["ai-dlc/run.sh", "-p", "{{{arg}}}"]
```

with `mise-tasks/ai-dlc/run.sh` passing the prompt to this skill's lifecycle. (`mise run` tasks are optional; you can run this skill directly with no task wiring.)
