...
- receiver-style vector helper lowering is intentionally minimal and explicit in this slice: `Vec3.Add(Vec3)` and `Vec3.Sub(Vec3)` lower directly to `OP_ADD_V`/`OP_SUB_V`, `Vec3.Mul(float32)` and `Vec3.Scale(float32)` both lower to `OP_MUL_VF`, and `Vec3.Dot(Vec3)` lowers to `OP_MUL_V` (dot product), all without `OP_CALL*` fallback. Other Vec3 methods remain deferred with `unsupported Vec3 method`.
- no-op type conversions whose source and target collapse to the same QCVM type (for example `*quake.Entity` ↔ `*buttonEntity`) are lowered as value-preserving conversions rather than normal calls, so receiver-adapter helper patterns in quakego compile without `*ast.StarExpr` fallback failures.
