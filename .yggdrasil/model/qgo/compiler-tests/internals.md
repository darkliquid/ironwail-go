...
- parity smoke harness now includes one deterministic Vec3 helper fixture (`vec3methods`) that calls `Compose(ax, ay, az, bx, by, bz, s)` and compares QCVM/native-Go results for a chained `Add→Sub→Scale→Dot` numeric path, while also pinning vector-op opcode presence (`OPAddV`, `OPSubV`, `OPMulVF`, `OPMulV`)
...
- vector method lowering tests now add a focused method-based slice: compile-level assertions require `OPAddV`/`OPSubV` with no `OPCall*` fallback for `a.Add(b).Sub(b)`, and a VM-visible round-trip verifies the resulting `OFS_RETURN` vector matches native-Go component math.
