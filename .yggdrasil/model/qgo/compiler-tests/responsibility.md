...
It also owns one deterministic end-to-end map-runner parity scenario (`MapRunner`) that executes a small loop/branch transition sequence in QCVM and compares return values with native-Go expectations.
It also owns one deterministic Vec3 helper parity fixture (`vec3methods/Compose`) that exercises `Add`/`Sub`/`Scale`/`Dot` in one numeric chain and compares QCVM output against equivalent native-Go math.
...
It also owns explicit deferred-scope evidence for advanced type forms: `x.(T)` and `switch v := x.(type)` are intentionally unsupported in this compiler slice and must fail with dedicated, stable diagnostics.
