...
The core runtime keeps commands and aliases in mutex-protected maps and stores buffered text in a `strings.Builder`. Buffered execution drains the current text snapshot, splits it on semicolons/newlines outside of quotes, tokenizes each line into argv-style slices, and resolves the first token by command, alias, then cvar. Alias expansion carries a recursion guard; `wait` reinserts the remaining lines at the head of the buffer; injected text is drained immediately so commands like `exec` or server stufftext can preempt the rest of the current pass.
