...
- root OpenGL alias shadow/static-pose vertex shaping now delegates the pure CPU mesh math to `internal/renderer/alias/mesh.go`, adapting `glAliasModel` pose/reference storage through package-local callbacks while keeping GL draw orchestration and cached alias resources local
