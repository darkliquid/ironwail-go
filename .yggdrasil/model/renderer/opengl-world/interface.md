...
- **Organization**: OpenGL backend implementations are organized in `internal/renderer/world_opengl.go` with true backend subpackage helpers under `internal/renderer/world/opengl/` for shader payloads, GL texture-upload primitives, sky texture resolution, sprite vertex flattening/final submission helpers, and CPU-only face bucketing/classification.
...
- root OpenGL code now delegates face bucketing/classification to `internal/renderer/world/opengl/bucket.go`, while retaining GL submission and renderer-owned state locally
- root OpenGL sprite rendering now delegates vertex flattening and final sprite draw submission to `internal/renderer/world/opengl/sprite.go`, while retaining cached sprite-model lookup, scratch-buffer ownership, and shared quad generation locally
- root OpenGL alias interpolation vertex shaping delegates pure CPU mesh math to `internal/renderer/alias/mesh.go`, with cached `glAliasModel` refs normalized to `[]aliasimpl.MeshRef` and consumed via `MeshFromRefs` while keeping GL draw orchestration and cached alias resources local
