...
OpenGL alias world-vertex construction now shares `internal/renderer/alias/mesh.go` with GoGPU: the root OpenGL path keeps cached alias-model ownership and GL submission, while `glAliasVertexRef` now satisfies the shared `MeshRefConvertible` contract so root code can hand alias refs to shared interpolation/rotation and vertex-shaping helpers without repeating local adapter closures.
...
- The remaining OpenGL root seam stops at renderer-owned cache lookup, pose selection, and GL submission; backend-local alias refs now expose shared mesh conversion directly instead of wrapping `MeshFromAccessor` inline at each call site.
