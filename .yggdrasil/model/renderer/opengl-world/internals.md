...
`world_render_opengl.go` remains behaviorally tied to those shared sky/runtime helpers; the recent edit there was compile-surface cleanup only, kept deliberately non-functional so renderer parity stays covered by focused package tests rather than hidden behavior changes.
