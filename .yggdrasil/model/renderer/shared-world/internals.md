...
The shared GoGPU world WGSL that still lives in `world.go` intentionally follows the OpenGL world-fragment contract for surface lighting: opaque world and turbulent/lit-water passes use the same `* 2.0` lightmap overbright factor as OpenGL, and both world-surface fog paths now use the same camera-distance exponential fog shape already used by the other GoGPU 3D passes instead of a backend-only constant fog blend. Keeping those formulas aligned prevents colored `.lit` maps such as qbj2 from rendering noticeably darker or more strongly tinted on GoGPU than on OpenGL.
Shared-world lightmap atlas assembly now also starts from black texels (with opaque alpha) instead of white texels before face-lightmap compositing. That matches the original C/OpenGL expectation for uninitialized atlas space more closely and avoids white-edge bleed when GPU sampling crosses unused or partially covered lightmap texels.
...
The shared world-pass cache now also keeps the camera-visible translucent world-liquid face subset alongside the sky/opaque/alpha/opaque-liquid batch data for each `(camera leaf, dynamic-light signature)` entry. GoGPU late translucency still recomputes per-frame distance sorting from those faces, but it no longer needs to rerun the full visible-face selection/filtering pass after the main world render already established the same visibility basis.
