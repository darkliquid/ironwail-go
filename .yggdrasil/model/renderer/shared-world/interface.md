...
- renderer world/entity render paths

## Contracts

- this node prepares or exposes the backend-neutral structures that concrete world renderers consume
- this node owns backend-neutral leaf-to-face lookup and PVS face selection so renderer implementations can consume the same visibility subset
...
- backend-neutral entity-lump parsing and alpha/bool cvar readers for worldspawn-driven sky/liquid behavior now also live under `internal/renderer/world/*`, so shared sky/liquid code consumes the same helper surface instead of reaching across backend files.
- canonical liquid-alpha policy now also lives under `internal/renderer/world/*`, including worldspawn liquid override parsing, cvar-backed liquid alpha resolution, and transparent-water VIS safety checks.
- canonical lightmap sample expansion now also lives under `internal/renderer/world/*`, with thin renderer-root wrappers preserving existing local helper names while the package split is still in progress.
