...
The backend core also now owns the lifetime of the most recent GoGPU world batch cache (leaf/light-signature key plus cached sky faces, batched indices, and batch descriptors). That keeps reuse state pinned to the live renderer instance while the shared-world slice decides when the cache key is valid.
That same renderer-owned cache state now also carries the cached translucent world-liquid face subset keyed by the same leaf/light signature. The backend core owns the lifetime of that reusable slice so the later entity/translucency phase can consume the main-world visibility result without rebuilding a second visibility worklist.
