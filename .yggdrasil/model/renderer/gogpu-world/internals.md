...
Those same opaque brush and opaque-liquid prep paths now avoid per-draw `VertexBytes` / `IndexBytes` heap allocations by packing directly into pooled contiguous byte slices (`appendGoGPUWorldVertexBytes`, `appendGoGPUWorldIndexBytes`) and issuing one scratch-buffer upload per payload before rendering.
...
GoGPU world shading now also splits the opaque-vs-alpha-test contract the way C Ironwail expects: the main opaque/turbulent world pipelines treat base-texture alpha as a lighting mask for regular embedded fullbright texels, while a dedicated alpha-test pipeline keeps discard semantics only for `{...}` cutout surfaces. That matches the shared world-texture upload policy where regular embedded fullbright texels stay in the base RGB with alpha `0`, and it prevents qbj/qbj2 world materials from being interpreted as partially transparent geometry just because they carry embedded fullbright details.
GoGPU sprite submission now keeps a second poster-only sprite pipeline for fully opaque `SPR_ORIENTED` wall posters. That pipeline uses the poster depth bias, enables depth writes, culls backfaces, and relies on sprite-alpha discard instead of ordinary alpha blending so wall posters behave more like Ironwail's opaque cutout sprite path. Ordinary sprites continue to use the original non-decal blended pipeline.
...
- Those same GoGPU hot paths now also consume cached shared-world liquid classification facts (`HasLitWater`, liquid-face-type mask) from `WorldGeometry` and brush draw DTOs instead of rescanning `[]WorldFace` every frame just to answer lit-water eligibility or "does this world contain translucent liquid?" questions.
...
- Root-local gogpu-tagged coverage in `internal/renderer/world_gogpu_brush_test.go` locks the new brush byte-pack append helpers to the same packed layout produced by shared `world/gogpu` compatibility packers.
...
- `renderAliasDrawsHAL` now reuses per-pass `[]WorldVertex` and `[]byte` scratch via `buildAliasVerticesInterpolatedInto` and `aliasVertexBytesInto`, wiring the shared non-alloc interpolation path into the GoGPU alias upload hotspot.
