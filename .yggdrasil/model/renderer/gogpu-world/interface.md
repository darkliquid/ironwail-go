...
- receiver-free alias CPU mesh/interpolation math now lives in `internal/renderer/alias/mesh.go`; the GoGPU root adapts cached alias pose/reference storage through shared `MeshFromAccessor` and keeps alias skin resolution, draw submission, and HAL ownership locally
