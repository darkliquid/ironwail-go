...
- receiver-free GoGPU decal helpers now live in `internal/renderer/world/gogpu/decal.go`; the root decal path keeps shared `buildDecalQuad` ownership, clamps final mark color/alpha, and keeps HAL/resource ownership, while delegating adapter-light mark collection, mark DTO shaping, batched packed draw preparation, uniform packing, and vertex byte shaping to the subpackage
