...
For CGO-free GoGPU builds, the concrete backend lives in `internal/renderer/gogpu`. It registers `gpucontext.EventSource` callbacks when available, but it also keeps a polling fallback for X11/plain-GoGPU sessions. That fallback must not call `gogpu`'s per-frame `Update()` a second time because the app already advances input state before `OnUpdate`; double-advancing would erase `JustPressed`/delta edges before the engine consumes them. The fallback preserves the engine-facing key vocabulary needed for console input, navigation, function keys, and numpad behavior, and it polls mouse buttons/scroll/position so degraded callback delivery still leaves the engine playable. The GoGPU backend routes engine mouse-grab toggles through a cursor adapter that controls platform cursor mode first: gameplay maps `CursorModeGrabbed` to `gpucontext.CursorModeLocked`, relying on gogpu's built-in hidden locked cursor behavior instead of separately forcing a cursor shape, while menus/console restore `CursorModeNormal` with the default visible cursor and `CursorModeHidden` keeps the cursor invisible without locking. Pointer-move handling prefers `gpucontext.PointerEventSource` so locked mode consumes `PointerEvent.DeltaX/DeltaY` directly; this avoids double-counting against the legacy `OnMouseMove` compatibility dispatch while still falling back to absolute-position deltas when pointer events are unavailable. The backend also guards cursor-mode reads and writes with its mutex so the event-thread pointer callback does not race against game-thread mode switches and accidentally treat locked motion as absolute center-warp updates.
