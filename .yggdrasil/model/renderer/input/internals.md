...
For CGO-free GoGPU builds, the concrete backend lives in `internal/renderer/gogpu`. It registers `gpucontext.EventSource` callbacks when available, but it also keeps a polling fallback for X11/plain-GoGPU sessions. That fallback must not call `gogpu`'s per-frame `Update()` a second time because the app already advances input state before `OnUpdate`; double-advancing would erase `JustPressed`/delta edges before the engine consumes them. The fallback preserves the engine-facing key vocabulary needed for console input, navigation, function keys, and numpad behavior, and it polls mouse buttons/scroll/position so degraded callback delivery still leaves the engine playable. The GoGPU backend now also routes engine mouse-grab toggles through its cursor-mode adapter, so gameplay hides the cursor with `CursorModeGrabbed` while menus/console restore `CursorModeNormal`; this keeps the renderer-owned backend aligned with the app-shell's existing `KeyGame` vs menu/console grab policy instead of ignoring grab requests entirely.
