...
`datagram.go` implements Quake's custom UDP reliability layer. Reliable sends copy the message into `sock.sendMessage`, send one fragment immediately, and clear `canSend`. ACK packets advance fragment state; once the final fragment is acknowledged, `canSend` becomes true again. Retransmission occurs after roughly one second without ACK. Incoming reliable data is ACKed immediately, accumulated until `FlagEOM`, and then returned as one completed message. Connection setup uses control packets: the client sends `CCReqConnect`, the server answers with `CCRepAccept` or `CCRepReject`, and accepted clients are switched onto a fresh per-client UDP socket whose port is returned in the accept reply. Before the normal accept path allocates that per-client socket, the datagram listener now checks the shared server IP-ban helper and emits the C rejection payload `You have been banned.\n` for matching IPv4 clients. Reconnects from the same remote endpoint close stale accepted sockets before replacement. The same control-packet handler now also answers `CCReqRuleInfo` by walking sorted `FlagServerInfo` cvars and returning the next `(name, value)` pair as `CCRepRuleInfo`.

`udp.go` keeps address parsing parity with C `UDP_StringToAddr`: when a connect target begins with a digit, it first routes through `PartialIPAddress` (from `net/discovery-support`) using `myTCPIPAddress` and `netHostPort` defaults; otherwise it resolves directly via `net.ResolveUDPAddr`, preserving ordinary hostname lookup behavior.
