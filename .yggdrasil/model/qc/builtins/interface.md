...
- `changelevel()` now prefers a server-owned `IssueChangeLevel` hook when available, allowing authoritative transition deduping (`ChangeLevelIssued`) before enqueueing `changelevel <map>`; if no hook is registered it falls back to command-buffer enqueue.
