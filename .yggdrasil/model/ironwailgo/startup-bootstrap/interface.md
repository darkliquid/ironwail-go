...
- Startup now wires `Host.SetGameDirChangedCallback(reloadRuntimeAfterGameDirChange)` so `game <mod>` switches trigger executable-side runtime rebuild (session teardown, server/QC/CSQC reload against the new VFS, runtime cache resets, draw/HUD refresh, and menu reset to main).
