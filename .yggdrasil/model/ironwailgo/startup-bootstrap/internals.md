...
Startup now installs a host game-dir-changed callback that runs executable-owned reload logic after host command `game` swaps the active VFS. That reload path intentionally preserves the live renderer/input stack while rebuilding mod-scoped runtime state: it tears down active sessions, creates a fresh server/QC/CSQC runtime, reloads `progs.dat`/CSQC from the new filesystem, resets visual/audio/model caches, refreshes draw/HUD assets from the new mod, and forces menu state back to main. To keep this safe with GoGPU's split update/draw threads, the callback now runs under the app-shell runtime-state lock and no longer pushes palette/conchars directly to the renderer from the update thread; instead it stages those assets for application on the render thread during `OnDraw`.
