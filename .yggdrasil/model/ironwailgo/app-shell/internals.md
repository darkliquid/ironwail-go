...
When CSQC is loaded, the runtime loop now treats CSQC HUD rendering as successful only when `CallDrawHud` both returns nil and reports `drewHUD=true` from QuakeC `OFSReturn`; otherwise it immediately falls back to native HUD rendering for that frame. This avoids a loaded-but-noop CSQC script suppressing native HUD overlays and keeps HUD visibility parity when CSQC opts out or fails.
