...
When `host_speeds` is enabled, `Server.Frame` now also emits a matching `server_speeds` info log that splits server time into new-client intake, client command ingestion, physics, rule enforcement, message send, and total wall-clock milliseconds. This keeps slow-map diagnosis aligned with the actual server-frame pipeline instead of forcing callers to guess which part of `ProcessServer()` consumed the host-frame budget.

When `host_speeds` is enabled, `Physics()` now also emits a `physics_speeds` info log that breaks the phase down further into StartFrame/QC sync time, force-retouch relinking, player Pre/PostThink QC time, per-movetype simulation buckets (`push`, `none`, `noclip`, `step`, `toss`, `walk`) with entity counts, send-interval bookkeeping, dev-stats counting, and total wall-clock milliseconds. This makes the next hotspot diagnosable without adding one-off profiling patches every time a mod map regresses.

Physics-owned player PreThink/PostThink calls now invoke the client-session QC helper in a no-full-sync mode. `Physics()` already performs a full `syncQCVMState()` before `StartFrame`; repeating another full-world sync immediately before the per-frame player think callbacks paid a Go-only cost that C does not. The physics path now re-syncs just the active player edict for those callbacks while leaving other non-physics client-session QC entry points on the conservative full-sync behavior.

`force_retouch` is sampled once per physics frame before the entity loop and then applied uniformly across entities when non-zero. The QC global only changes at frame boundaries in this flow, so re-reading it by name for every edict added avoidable per-frame VM lookup overhead without changing behavior.
