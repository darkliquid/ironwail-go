...
- `game` now provides the runtime mod-switch seam used by menu mods-browser selection: no args prints current gamedir, one arg validates/switches filesystem mount to the selected gamedir (`id1` or discovered mods), runs host runtime's optional game-dir-changed callback with the freshly mounted `*fs.FileSystem` and already-swapped `subs.Files` pointer (used by executable wiring to rebuild mod-scoped runtime state), and invalid names print usage/error text without mutating the active VFS.
