...
| High-priority remaining gaps | **integration/fidelity (external skybox/pass-order, remote connect transport flow, load UX)** |
...
1. **External skybox + pass-order fidelity** → visual sequencing still diverges from C (turbulent UV warp and embedded sky layers are now wired on OpenGL)
