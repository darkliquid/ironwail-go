...
- replace remaining TODO submenu actions for join/host/controls (bounded player setup menu now supports name + shirt/pants colors + accept/apply via `name`/`color`; options now routes to bounded video/audio/controls submenus backed by live cvars and bind commands)
