...
- particle rendering now has explicit opaque/translucent subpass plumbing in the top-level OpenGL frame path, world/brush liquid surfaces now bucket into dedicated opaque/translucent liquid bins, and frame-level ordering now gives world/brush liquid buckets their own stage between opaque work and later decal/particle/viewmodel steps; the broader scene ordering is still simpler than `R_RenderScene()` and sky/viewmodel/translucency sequencing still diverges
